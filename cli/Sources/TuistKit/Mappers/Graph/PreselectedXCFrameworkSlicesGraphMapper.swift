...
struct PreselectedXCFrameworkSlicesGraphMapper: GraphMapping {
    ...
    private func collect(
        ...
        for variant in variants {
            guard let selectedFramework = Self.selectedFramework(
    ...
    private static func selectedFramework(
        ...
        guard FileManager.default.fileExists(atPath: frameworkPath.pathString) else { return nil }
    ...
    private func runtimeScript(plan: TargetPlan) -> TargetScript? {
        var runtimeFrameworksBySDK: [String: [AbsolutePath]] = [:]
        for (variant, sdkPlan) in plan.sdkPlans {
            ...
            let runtimeFrameworks = sdkPlan.frameworksByName.values
                .filter(\.runtime)
                .map { frameworkUse in
                    frameworkDirectory.appending(component: "\(frameworkUse.framework.frameworkName).framework")
                }
        ...
        let cases = runtimeFrameworksBySDK.keys.sorted().map { sdk -> String in
            let installCommands = runtimeFrameworksBySDK[sdk, default: []]
                .map { frameworkPath -> String in
                    let relativePath = frameworkPath.relative(to: plan.project.sourceRootPath).pathString
        ...
        return TargetScript(
            ...
            script: .embedded(
                ...
                install_framework() {
                  ...
                  rsync -a --delete "$source/" "$destination_root/$name"
                  if [ "${CODE_SIGNING_ALLOWED:-YES}" != "NO" ] && [ -n "${EXPANDED_CODE_SIGN_IDENTITY:-}" ]; then
                    codesign --force --sign "$EXPANDED_CODE_SIGN_IDENTITY" --preserve-metadata=identifier,entitlements "$destination_root/$name"
                  fi
                }
